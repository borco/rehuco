...
- **Before B0 promises "browser plays the video":** an iPad-playback **spike** — serve a representative sample of the real catalog to the actual tablet: container/codec coverage (Safari plays H.264/HEVC in MP4/MOV; MKV — common in tutorial catalogs — does not play natively), the self-signed-HTTPS trust story (§A03.2), and HTTP Range seeking. The outcome decides whether Milestone B grows a remux/transcode task-queue job.
