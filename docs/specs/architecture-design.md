...
- The node and agent are installable as tools: **`uv tool install rehuco-node`** (ideal — headless service, console entry point) and **`uv tool install rehuco-agent`** (works for the GUI; native installers / file-association registration (§5.4) are a later polish for wider distribution, not needed for the author's own machines).
