...
| ReferenceImages type (`.cbz`, samples) | Yes | A6 (basic type + fields + viewer); redaction/search/slideshow deferred past D |
...
| Per-site scrapers | Yes (6 sites) | deferred past D; the geckodriver+BeautifulSoup approach is explicitly the cautionary predecessor, with an LLM URL-extraction successor |
