...
> [!NOTE]
> No tagged release has been cut yet. `release-agent.yml` has so far only been exercised through
> `workflow_dispatch` dry runs, which build every artifact but publish nothing.
...
cannot create or modify a Release no matter how it goes.
...
1. Bump `__version__` in `packages/rehuco-agent/src/rehuco_agent/__init__.py`.
2. Commit it. On `master` the message carries no `refs` prefix; `repo:` is the fitting type prefix for a
   release chore.
...
## 4. What the tagged run does, and how to redo it
...
GitHub Release or, if one already exists for that tag, uploads over it with `--clobber`. Re-running the same
...
## 6. Windows packaged builds print nothing
