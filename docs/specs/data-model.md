...
- **A resource checksummed before this app existed is verified, not baselined** (#243). The catalog carries `.sfv`
  ...
  - **A manifest is retired once its claim is absorbed** (#259) — the run that wrote the record renames `info.sfv` to
    ...
    backup suffix ([[acquisition-tooling#convert-mechanics]]), so a retired manifest joins the resource's retained
    backup pair for Discard with no new vocabulary, and the content walk already skips it. No marker is needed and none
...
- **A record written before its manifest was absorbed is remediated, not left** (#259). What retirement leaves is a
  ...
  **inherited, not built** — a line naming something the enumeration leaves out never becomes an entry at all, so it
  ...
  narrow**: the seeding path no longer produces the state, and the door it once recurred through — a revert
  restoring a retired manifest wholesale beside the record it deliberately kept — is gone along with Revert itself
  ([[acquisition-tooling#convert-mechanics]]). The state now only ever originates in a catalog hand-converted before
  retirement existed, never regenerated by anything the app still does. So the merge takes no options and
...
- **Two tiers of exclusion, and only one is the user's.** **Structural** — **every record** a scan meets, at any
  ...
  **A retained `.orig` conversion backup is
  ...
  look it up against, and **any** `.orig` counts. That definition is asked of the backups module rather than restated
  here. A bulk import retains this small backup pair by default, so
  counting it would bake each converted resource's own `info.tc.orig` into its first checksum baseline, and
  discarding it afterwards — the encouraged cleanup — would then report a missing file for every resource in
...
[[nodes#local-vs-swarm]]): parsing enforces sanity caps (`rehu_parse_limits`, #88) — total file size, entry
counts and sizes (covering a future `versions` list the same as any other collection), JSON nesting depth
— so a file exceeding them is refused at load, `RehuDocument.load` raising `RehuFormatError`. The normal
open path (`RehuDocument.open_or_locked`) turns that refusal into a read-only view with a warning rather
than a dialog, the same as any other unparseable file, instead of exhausting memory or wedging the app.
