...
[[nodes#local-vs-swarm]]): parsing will enforce the planned sanity caps from #88 — total file size, `versions`-list
length, entry sizes, and JSON nesting depth — so a file exceeding them opens read-only with a warning (or is refused
at import) instead of exhausting memory or wedging the app.
