"""A two-line `View` menu entry for an open document dock: its title above its dimmed, right-elided path."""
...
MAX_WIDTH: Final = 600
"""Maximum pixel width of a single entry, so a long title or path elides instead of growing the
`View` menu without bound."""
...
class DocksMenuEntry(QWidget):
    """A `View` menu entry for an open document dock: its title in normal text, its full path
    beneath it in smaller, dimmed text -- both right-elided to fit :data:`MAX_WIDTH`. Built as a
    plain (non-interactive) widget -- neither label grabs mouse events, so a click still falls
    through to the wrapping `QMenu`, which triggers the entry's `QWidgetAction` for free.

    :param title: the dock's display title (`RehuDocumentModel.label`).
