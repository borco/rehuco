...
Unlike rasterizing once and recoloring pixels, this rewrites the SVG's own fill color and keeps it
as a real, infinitely-scalable icon -- exactly as crisp at any requested size as an untouched
SVG-backed ``QIcon``, since nothing is ever cached at a fixed resolution. Only works correctly for
a genuinely monochrome source (see
