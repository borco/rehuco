...
def test_rating_starts_empty_by_default(qtbot: QtBot) -> None:
    """A freshly built ``Rating`` with no starting value is unrated (``None``) and shows nothing.
...
def test_rating_shows_nothing_for_none(qtbot: QtBot) -> None:
    """``None`` (unrated) shows nothing, the same as a genuine zero -- even after showing stars before.
