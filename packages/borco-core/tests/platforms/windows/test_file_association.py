...
@mark.windows
def test_is_registered_is_false_when_the_extension_points_elsewhere(fake_registry: FakeRegistry) -> None:
    """``is_registered`` reports ``False`` when the extension's default value points at a different
    ProgID now (another app claimed the extension since).

    **Test steps:**

    * register, then repoint the extension's default value at a different ProgID
    * check ``is_registered`` with the original arguments
    * verify ``False``
    """
    file_association.FileAssociation.register(PROGID, EXTENSION, FRIENDLY_NAME, COMMAND, ICON, AUMID)
    ext_key = rf"Software\Classes\.{EXTENSION}"
