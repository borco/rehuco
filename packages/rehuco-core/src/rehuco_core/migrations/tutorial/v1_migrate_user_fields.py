"""**tutorial block v0 -> v1**: nest the per-user fields under the block's ``users`` map
([[field-schema#per-user-shared]]).

Self-contained: the field set and the map key are frozen *here*, at v1, and passed to the shared
:func:`~rehuco_core.migrations.shared.migrate_user_fields.migrate_user_fields` mechanism. A later change to
another plugin's per-user set -- or a rename of the ``users`` key in some future version -- never touches
...
VERSION = 1
"""The version this step brings a ``tutorial`` block up to."""
...
def upgrade(block: dict[str, Any], username: str) -> None:
    """Relocate the tutorial block's per-user fields under ``users[username]``, in place.

    :param block: one v0 ``tutorial`` block's own fields; mutated in place.
