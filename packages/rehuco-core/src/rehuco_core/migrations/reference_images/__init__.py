...
which is the divergence the per-target declaration was written to allow. **v4** is this block's own too
-- ``images_count`` becomes ``current_count``, the measured half of the count pair (#198).
``CURRENT_VERSION`` is the chain's head, derived.
"""
...
from . import v1_migrate_user_fields, v2_learning_path_refs, v3_drop_progress_flags, v4_rename_images_count
...
CHAIN = (
    ...
    (v4_rename_images_count.VERSION, v4_rename_images_count.upgrade),
)
