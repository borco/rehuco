"""Tests for LocationTemplatePatternsEditor: the one-column patterns editor's own behaviour (#322)."""

# Almost line for line test_screenshot_name_patterns_editor.py's own -- the two production editors are
...
@fixture(name="editor")
def fixture_editor(qtbot: QtBot) -> LocationTemplatePatternsEditor:
    """An editor holding the shipped patterns.

    :param qtbot: pytest-qt fixture, which owns the widget's lifetime.
    :returns: the editor.
    """
    editor = LocationTemplatePatternsEditor()
    qtbot.addWidget(editor)
    editor.values = NAME_SUGGESTION_PATTERNS
...
def test_the_editor_is_the_shared_list_machinery_over_a_table(editor: LocationTemplatePatternsEditor) -> None:
...
def test_values_round_trip_exactly_as_typed(editor: LocationTemplatePatternsEditor) -> None:
    ...
    * verify they come back untouched
    """
    patterns = (" {authors} - {title} ",)
    editor.values = patterns

    assert editor.values == patterns


def test_reset_is_hidden_when_there_is_nothing_to_restore(editor: LocationTemplatePatternsEditor) -> None:
...
def test_delete_edits_the_text_while_a_cell_is_open(editor: LocationTemplatePatternsEditor) -> None:
    ...
    * verify a character went and every pattern is still there
    ...
    assert line.text() == NAME_SUGGESTION_PATTERNS[0][1:]
    assert len(editor.values) == before


def test_a_click_acts_on_one_whole_pattern(editor: LocationTemplatePatternsEditor) -> None:
...
def test_the_column_takes_the_full_width(editor: LocationTemplatePatternsEditor) -> None:
