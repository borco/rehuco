"""Tests for ConversionBackupsDialogSettings: the dialog's geometry and its recent-roots MRU list (#193).

Uses a hand-rolled in-memory stand-in for ``QSettings`` rather than a real one or ``tmp_path`` -- the
same array-capable shape `test_import_legacy_catalog_wizard_settings` uses, since this settings object
mixes a scalar (geometry) with an array (recent roots).
"""
...
from rehuco_agent.settings.conversion_backups_dialog_settings import (
    GROUP,
    MAXIMUM_RECENT_ROOTS,
    ConversionBackupsDialogSettings,
)
from rehuco_agent.settings.import_legacy_catalog_wizard_settings import GROUP as WIZARD_GROUP
...
# region fixtures
# Mirrors test_import_legacy_catalog_wizard_settings.py's own FakeSettings exactly (same array-capable
# QSettings stand-in) -- kept as a separate copy rather than a shared import, matching this codebase's
...
def test_record_root_appends_a_new_root_as_the_newest() -> None:
    ...
    * verify ``newest_roots_first`` reports the second one first
    """
    dialog_settings = ConversionBackupsDialogSettings()
    dialog_settings.record_root(FIRST)
    dialog_settings.record_root(SECOND)

    assert dialog_settings.newest_roots_first() == [SECOND, FIRST]
...
def test_record_root_moves_an_already_remembered_root_to_newest() -> None:
    ...
    * verify it's now newest, and the list has no duplicate entry
    ...
    assert dialog_settings.newest_roots_first() == [FIRST, SECOND]
...
def test_record_root_drops_the_oldest_entry_past_the_cap() -> None:
    ...
    * verify the oldest one is gone and the count matches the cap
    """
    dialog_settings = ConversionBackupsDialogSettings()
    roots = [Path.cwd() / "fake" / str(index) for index in range(MAXIMUM_RECENT_ROOTS + 1)]
    for root in roots:
        dialog_settings.record_root(root)

    newest_first = dialog_settings.newest_roots_first()
...
def test_save_then_load_round_trips_geometry_and_roots(settings: FakeSettings) -> None:
    ...
    * verify both round-tripped
    """
    dialog_settings = ConversionBackupsDialogSettings(geometry=b"blob")
    dialog_settings.record_root(FIRST)
    dialog_settings.record_root(SECOND)

    dialog_settings.save(settings)  # type: ignore[arg-type]

    restored = ConversionBackupsDialogSettings()
...
def test_the_roots_do_not_share_the_import_wizards_list(settings: FakeSettings) -> None:
    """A folder worth importing from and a folder worth cleaning up are the same one only until they
    are not, so browsing in one dialog must never reorder the other's list.

    **Test steps:**

    * save a root from this dialog's settings
    * verify every key written sits under this dialog's own group, not the wizard's
    """
    dialog_settings = ConversionBackupsDialogSettings()
    dialog_settings.record_root(FIRST)
    ...
    assert GROUP != WIZARD_GROUP
    assert all(key.startswith(f"{GROUP}/") for key in settings.keys()), settings.keys()
