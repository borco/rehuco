...
class ScraperExecutor:
    """Runs each `ScrapeJob` on its own worker, independent of every other scrape and of the app-wide
    `rehuco_core.TaskQueue`'s single worker.
    ...
    **Carries the caller's `borco_core.logging.LogScope` onto the worker.** A pool thread inherits
    ...
    class Runnable(QRunnable):
        """Runs one `ScrapeJob` inside the log context captured at submission.
        ...
        -- the job's `ScrapeJob.Marshaller` must outlive the emit its `ScrapeJob.run` makes at the end.
        """

        def __init__(self, job: ScrapeJob, context: Context) -> None:
            super().__init__()
    ...
    def submit(self, job: ScrapeJob) -> None:
        """Run ``job`` on the pool, carrying the caller's log scope onto the worker thread.

        :param job: the scrape to run. Nothing about one submission blocks another -- each runs as an
            independent `QRunnable`, and nothing on the app-wide `rehuco_core.TaskQueue` shares this
            pool at all. **The caller keeps its reference to ``job`` until `ScrapeJob.result_ready`
            has fired**: the runnable is auto-deleted the moment ``run`` returns, which is before the
            queued signal is delivered, so nothing on this side holds the job's marshaller alive past
            that point.
        """
